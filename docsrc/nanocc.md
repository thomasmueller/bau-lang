...
NanoCC is a self-hosting C-like compiler (~250 lines) 
that compiles directly to WebAssembly.
This project explores how much of C can be removed
while remaining self-hosting and readable.

Example (reads from stdin and echoes the input):
...
* Variables of a single data type: 64-bit integers
* Number literals: decimal `0`, hex `0xcafe`
* Multi-byte character literals (up to 8 characters),
  packed into an integer: `x = 'hello';`
* Two built-in functions: `getchar()` and `putchar(x)`
* Variable assignment: `i = 16;`
  Variables are implicitly declared on first assignment
* Memory access, using integers as pointers: `*i = 0xcafe;`
* Control flow: `if`, `else`, `while`
* Operators `+ - << >> & | == != < <= ()` are evaluated from left to right (no precedence)
* `// line comments`
...
* No advanced features like arrays, operator precedence, structs, return, break, continue.
* No string literals.

## How it Works

The compiler first tokenizes the input into a flat buffer.
It then uses a stack machine to convert the tokens to variable-size instructions.
At the end, it emits the instructions (including the WebAssembly header) as bytes.
...
* To run WASM bytecode, you need [wasmtime](https://github.com/bytecodealliance/wasmtime).
* For bootstrapping NanoCC, you need Java. `mvn clean test`. This will generate `boot.wasm`.
* Run `cat nanocc.c | wasmtime boot.wasm > nanocc.wasm` to create the first version.
* Run `cat nanocc.c | wasmtime nanocc.wasm > nanocc2.wasm` to create the second version.

## Related

* [SectorC](https://github.com/bytecodealliance/wasmtime), a C compiler written in x86-16 assembly (512 bytes).
* [Obfuscated Tiny C Compiler](https://bellard.org/otcc/), obfuscated 2048 bytes of C source, otherwise around 450 lines.
* [C4](https://github.com/rswier/c4), C in 4 functions, 500 lines.
* [cc500](https://github.com/8l/cc500), a small self-hosting C compiler, 700 lines.
* [nanoc](https://github.com/AjayMT/nanoc), small subset of C and compiler, 1600 lines.
