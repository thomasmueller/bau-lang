...
import static org.bau.mini.vm.reg.RegBytecode.*;
...
public final class RegCompiler {
    ...
    public RegBytecode compile(Stmt.Program program) {
        ...
        if (!funcIndex.containsKey("main")) {
            Stmt.Block top = new org.bau.mini.ast.Stmt.Block(program.items(), 1, 1);
    ...
    private void validateCallsInStmt(Stmt s) {
        if (s instanceof Stmt.Assign a) {
            ...
            validateCallsInExpr(u.target());
            validateCallsInExpr(u.value());
            ...
            validateCallsInExpr(fr.limit());
            validateCalls(fr.body());
    ...
    private void validateFieldsInStmt(Stmt s) {
        if (s instanceof Stmt.Assign a) {
            ...
            validateFieldsInExpr(u.target());
            validateFieldsInExpr(u.value());
        } else if (s instanceof Stmt.ExprStmt e) {
            ...
            validateFieldsInExpr(fr.limit());
            validateFields(fr.body());
        } else if (s instanceof Stmt.Return r) {
            if (r.value() != null)
    ...
    private void validateTypesAndVarsInStmt(Stmt s) {
        if (s instanceof Stmt.Assign a) {
            ...
            validateTypesAndVarsInExpr(u.target());
            validateTypesAndVarsInExpr(u.value());
        } else if (s instanceof Stmt.ExprStmt e) {
            ...
            validateTypesAndVarsInExpr(fr.limit());
            validateTypesAndVars(fr.body());
        } else if (s instanceof Stmt.Return r) {
            if (r.value() != null)
    ...
    private boolean stmtHasCalls(Stmt s) {
        ...
        if (s instanceof Stmt.Update u) {
            return exprHasCalls(u.target()) || exprHasCalls(u.value());
        }
        ...
        if (s instanceof Stmt.For fr) {
            return exprHasCalls(fr.limit()) || blockHasCalls(fr.body());
        }
    ...
    private void validateReturnsInStmt(Stmt s, Type expected, String fname) {
        if (s instanceof Stmt.Return r) {
            ...
            validateReturnsInBlock(fr.body(), expected, fname);
        }
    }
    ...
    private void compileStmt(RegBuilder b, Stmt s) {
        if (s instanceof Stmt.Assign a) {
            ...
            int lv = compileExpr(b, u.target());
            int rv = compileExpr(b, u.value());
            Type lt = u.target() instanceof Expr.Variable v ? v.type() : u.target().type();
            if (lt instanceof Type.Text) {
                throw new RuntimeException("text update '+:' is not supported; use format(...)");
            } else if (lt instanceof Type.Array) {
                throw new RuntimeException("array concatenation/update is not supported");
            } else if (lt instanceof Type.Int) {
                // enforce right is int
                if (!(u.value().type() instanceof Type.Int))
                    throw new RuntimeException("type error: update int with non-int");
                int tmp = b.alloc();
                b.emit(Insn.op3(Op.ADD, tmp, lv, rv));
                assignTarget(b, u.target(), tmp);
                b.free(tmp);
            } else if (lt instanceof Type.Real) {
                if (!(u.value().type() instanceof Type.Real))
                    throw new RuntimeException("type error: update real with non-real");
                int tmp = b.alloc();
                b.emit(Insn.op3(Op.FADD, tmp, lv, rv));
                assignTarget(b, u.target(), tmp);
                b.free(tmp);
            } else {
                throw new RuntimeException("type error: unknown lvalue type in update");
            }
        } else if (s instanceof Stmt.ExprStmt e) {
            ...
            assignTarget(b, new Expr.Variable(fr.var(), 0, 0), zero);
            b.free(zero);
            List<Integer> breaks = new java.util.ArrayList<>();
            breakStack.push(breaks);
            loopDepth++;
            int start = b.f.code.size();
            int iv = compileExpr(b, new Expr.Variable(fr.var(), 0, 0));
            int lim = compileExpr(b, fr.limit());
            int cmp = b.alloc();
            b.emit(Insn.op3(Op.LT, cmp, iv, lim));
            b.free(iv);
            b.free(lim);
            int jmpToBody = b.f.code.size();
            b.emit(Insn.jmpif(cmp, 0));
            int jmpEnd = b.f.code.size();
            b.emit(Insn.jmp(0));
            int bodyStart = b.f.code.size();
            b.f.code.set(jmpToBody, Insn.jmpif(cmp, bodyStart));
            b.free(cmp);
            compileBlock(b, fr.body());
            int one = b.alloc();
            b.emit(Insn.loadi(one, 1));
            int cur = compileExpr(b, new Expr.Variable(fr.var(), 0, 0));
            int inc = b.alloc();
            b.emit(Insn.op3(Op.ADD, inc, cur, one));
            b.free(cur);
            b.free(one);
            assignTarget(b, new Expr.Variable(fr.var(), 0, 0), inc);
            b.free(inc);
            b.emit(Insn.jmp(start));
            int end = b.f.code.size();
            b.f.code.set(jmpEnd, Insn.jmp(end));
