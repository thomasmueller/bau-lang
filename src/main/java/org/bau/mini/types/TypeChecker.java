...
public final class TypeChecker {
    ...
    private void inferStmt(Stmt st, Scope s, Map<Expr, Type> exprT, Map<Stmt, Type> stmtT) {
        if (st instanceof Stmt.Assign a) {
        ...
        } else if (st instanceof Stmt.Update u) {
            Type lt = inferExpr(u.target(), s, exprT);
            inferExpr(u.value(), s, exprT);
            stmtT.put(st, lt);
            ...
            stmtT.put(st, Type.VOID);
        } else if (st instanceof Stmt.For fr) {
            // Declare loop variable as int within this scope
            s.vars.put(fr.var(), Type.INT);
            inferExpr(fr.limit(), s, exprT);
            // Infer body to propagate types within the loop
            inferBlock(fr.body(), s, exprT, stmtT);
            stmtT.put(st, Type.VOID);
