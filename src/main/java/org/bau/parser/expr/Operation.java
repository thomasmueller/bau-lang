...
public class Operation implements Expression {
    ...
    public String toC() {
        ...
        if (">>".equals(op)) {
            ...
            return FullName.esc("shiftRight") + "_" + t.name() + "_2(" +
                    left.toC() + ", " + right.toC() + ")";
        } else if ("<<".equals(op)) {
            return FullName.esc("shiftLeft") + "_2(" +
                    left.toC() + ", " + right.toC() + ")";
        } else if ("/".equals(op)) {
            if (widerType().isFloatingPoint()) {
                return left.toC() + " / " + right.toC();
            } else {
                return FullName.esc("idiv") + "_2(" +
                        left.toC() + ", " + right.toC() + ")";
            }
        } else if ("%".equals(op)) {
            return FullName.esc("imod") + "_2(" +
                    left.toC() + ", " + right.toC() + ")";
    ...
    public void used(Program program) {
        ...
        case ">>":
            ...
            program.getFunction(null, "", "shiftRight_" + t.name(), 2).used(program);
            break;
        case "<<":
            program.getFunction(null, "", "shiftLeft", 2).used(program);
            break;
        case "/":
            if (widerType().isFloatingPoint()) {
                // floating point
            } else {
                program.getFunction(null, "", "idiv", 2).used(program);
            }
            break;
        case "%":
            program.getFunction(null, "", "imod", 2).used(program);
            break;
        }
        right.used(program);
    }
        ...
        if (expr instanceof Variable) {
            ...
            if (f.base.type().isArray() && f.fieldName.equals("len")) {
                if (f.base instanceof Variable) {
                    ...
                    return Solver.variable(f.format());
                }
            } else {
                return Solver.variable(f.format());
            }
