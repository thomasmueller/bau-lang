...
public class Call implements Statement, Expression, LeftValue {
    ...
    public Value eval(Memory m) {
        if (def.isFunctionPointer) {
            String name = def.getFullName().name;
            if (m == null) {
                return null;
            }
        ...
        if (def.cCode != null) {
            Value result = Std.eval(def.getFullName().name, m);
    ...
    public StatementResult run(Memory m) {
        if ("println".equals(def.getFullName().name)) {
    ...
    @Override
    public String toC() {
        if (def.callType == null && "println".equals(def.getFullName().name)) {
            return printlnToC();
        }
        if (statement) {
    ...
    public String callToC() {
        StringBuilder buff = new StringBuilder();
        if (!def.getFullName().module.isEmpty()) {
            buff.append(FullName.esc(def.getFullName().module).replace('.', '_') + "_");
        }
