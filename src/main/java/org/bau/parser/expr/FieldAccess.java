...
public class FieldAccess implements Expression, LeftValue {
    ...
    public Expression replace(Variable old, Expression with) {
        if (fieldName.equals("source")
                ...
                && old.name().startsWith(base.format() + ".")) {
            return with;
        } else if (fieldName.equals("ast")
                && old.name().endsWith(".ast")
                && old.name().startsWith(base.format() + ".")) {
            return with;
        } else if (fieldName.startsWith("values")
    ...
    public String toC() {
        if (base.type().isArray()) {
            ...
            return base.toC() + "->" + FullName.esc(fieldName);
        } else {
            return base.toC() + "." + FullName.esc(fieldName);
        }
    }
    ...
    public List<Rule> getRules() {
        if ("len".equals(fieldName)) {
            Rule r = Solver.rule(Solver.variable(format()), ">", Solver.number(0));
            return List.of(r);
        }
        return List.of();
    }
