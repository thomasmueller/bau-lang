...
int64_t solve_1(i8_array* c) {
    ...
    while (k <= 57) {
        int64_t j = 0;
        while (j < 9) {
            int64_t m = (idiv_2(i, 10)) * 9 + ( imod_2(i, 10) );
            if (c->data[idx_2(((idiv_2(m, 9)) * 10) + j, _arrayLen(c))] == k) {
                break;
            }
            if (c->data[idx_2((imod_2(m, 9)) + ( 10 * j ), _arrayLen(c))] == k) {
                break;
            }
            int64_t h = ((imod_2((idiv_2(m, 27)), 3)) * 3) + (idiv_2((imod_2(m, 9)), 3));
            int64_t n3 = ( h * 3 ) + (idiv_2(h, 3)) * 18 + (imod_2(j, 3)) + (idiv_2(j, 3)) * 9;
...
int main(int _argc, char *_argv[]) {
    ...
    __argc = _argc;
    __argv = _argv;
    string_1000 = str_const("Expected one argument: \"<line1> <line2> ... <line9>\"", 52);
    string_1001 = str_const("where each line is 9 characters: 0-9, or . for unknown.", 55);
    string_1012 = str_const("No solution found", 17);
    _main();
    return 0;
}
void _main() {
