...
void Tree_owned_free_0(Tree_owned* x) {
...
void Tree_owned_free(Tree_owned* x) {
    if (x == NULL) return;
    _registerAndMaybeDrain(x, (void(*)(void*))Tree_owned_free_0);
}
...
Tree_owned* with_1(int64_t depth) {
    if (depth == 0) {
        Tree_owned* _t0 = Tree_2(NULL, NULL);
        return _t0;
    }
    Tree_owned* _t1 = with_1(depth - 1);
    Tree_owned* _t2 = with_1(depth - 1);
    Tree_owned* _t3 = Tree_2(_t1, _t2);
    _t1 = NULL;
    _t2 = NULL;
    Tree_owned_free(_t2);
    Tree_owned_free(_t1);
...
void _main() {
    ...
    if (stretch != NULL) {
        int64_t _t1 = Tree_owned_nodeCount_1(stretch);
