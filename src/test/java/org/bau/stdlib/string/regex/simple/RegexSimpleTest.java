...
public class RegexSimpleTest {
    ...
    private static void assertMatches(String text, String regex) {
        ...
        boolean got = RegexSimple.matches(text, regex);
        if (expected != got) {
            ...
            for (int m = 0; m <= 1; m++) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (m == 0) {
                        ...
                        matchCount += RegexSimple.matches(s, regex) ? 1 : 0;
                    } else {
    ...
    @Test
    public void replace() {
        assertEquals("bXbadddb", RegexSimple.replaceFirst("bbabadddb", ".a", "X"));
        assertEquals("c", RegexSimple.replaceFirst("c", ".{2,2}", "X"));
        assertEquals("baNanna", "bannanna".replaceFirst("n+", "N"));
        assertEquals("baNanna", RegexSimple.replaceFirst("bannanna", "n+", "N"));
        assertEquals("baNaNa", "bannanna".replaceAll("n+", "N"));
        assertEquals("baNaNa", RegexSimple.replaceAll("bannanna", "n+", "N"));
    }

    @Test
    public void split() {
        assertEquals("[a, b, c]", Arrays.toString("a, b,   c".split(", *")));
        assertEquals("[a, b, c]", Arrays.toString(RegexSimple.split("a, b,   c", ", *")));
    }
    ...
    public void random() {
        ...
        for (int test = 0; test < count; test++) {
            ...
            String regex = randomRegex(r, 9, false);
            if (r.nextInt(5) == 0) {
                if (r.nextBoolean()) {
                    regex = "^.*" + regex;
                } else {
            ...
            boolean got;
            try {
                got = RegexSimple.matches(text, regex);
            ...
            String g = RegexSimple.replaceFirst(text, regex, "X");
            if (!exp.equals(g)) {
            ...
            String ga = RegexSimple.replaceAll(text, regex, "X");
            if (!expa.equals(ga)) {
                if (regex.length() < smallestReplaceFirst) {
                    smallestReplaceFirst = regex.length();
