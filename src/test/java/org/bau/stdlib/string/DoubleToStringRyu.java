...
public class DoubleToStringRyu {
    ...
    static {
        ...
        for (int i = 0; i < Math.max(POW5.length, POW5_INV.length); i++) {
            ...
            if (i < POW5_SPLIT.length) {
                for (int j = 0; j < 4; j++) {
                    POW5_SPLIT[i][j] = pow.shiftRight(pow5len - POW5_BITCOUNT + (3 - j) * POW5_QUARTER_BITCOUNT)
                            .and(mask).intValueExact();
                }
            }
            ...
            if (i < POW5_INV_SPLIT.length) {
                ...
                for (int k = 0; k < 4; k++) {
                    if (k == 0) {
                        ...
                        POW5_INV_SPLIT[i][k] = inv.shiftRight((3 - k) * POW5_INV_QUARTER_BITCOUNT).and(invMask)
                                .intValueExact();
                    }
                }
            }
    ...
    public static String doubleToString(double value) {
        ...
        // Step 4: Find the shortest decimal representation in the interval of legal
        // representations.
        //
        // We do some extra work here in order to follow Float/Double.toString
        // semantics. In particular,
        // that requires printing in scientific format if and only if the exponent is
        // between -3 and 7,
        // and it requires printing at least two decimal digits.
        //
        // Above, we moved the decimal dot all the way to the right, so now we need to
        // count digits to
        // figure out the correct exponent for scientific notation.
        final int vplength = decimalLength(dp);
        int exp = e10 + vplength - 1;

        // Double.toString semantics requires using scientific notation if and only if
        // outside this range.
        ...
        if (scientificNotation) {
            ...
            // Print 'E', the exponent sign, and the exponent, which has at most three
            // digits.
            result[index++] = 'E';
            if (exp < 0) {
                result[index++] = '-';
    ...
    private static long mulPow5divPow2(long m, int i, int j) {
        ...
        return ((((((((bits00 >>> 31) + bits01 + bits10) >>> 31) + bits02 + bits11) >>> 31) + bits03 + bits12) >>> 21)
                + (bits13 << 10)) >>> actualShift;
    }

    /**
     * Compute the high digits of m / 5^i / 2^j such that the result is accurate to
     * at least 9 decimal digits. i and j are already chosen appropriately.
     */
