...
public class SHA256 {
    ...
    private static final int[] INIT = new int[] { 0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab,
            0x5be0cd19 };
    ...
    public static byte[] getHash(byte[] data) {
        ...
        for (int i = 0, j = 0; j < intLen; i += 4, j++) {
            buff[j] = ((bytes[i] & 0xff) << 24) + ((bytes[i + 1] & 0xff) << 16) + ((bytes[i + 2] & 0xff) << 8) + (bytes[i + 3] & 0xff);
        }
        buff[intLen - 2] = byteLen >>> 29;
        buff[intLen - 1] = byteLen << 3;
        int[] w = new int[64];
        int[] hh = Arrays.copyOf(INIT, INIT.length);
        for (int block = 0; block < intLen; block += 16) {
            ...
            for (int i = 16; i < 64; i++) {
                int x = w[i - 2];
                int theta1 = Integer.rotateRight(x, 17) ^ Integer.rotateRight(x, 19) ^ (x >>> 10);
                x = w[i - 15];
                int theta0 = Integer.rotateRight(x, 7) ^ Integer.rotateRight(x, 18) ^ (x >>> 3);
                w[i] = theta1 + w[i - 7] + theta0 + w[i - 16];
            }
            int a = hh[0], b = hh[1], c = hh[2], d = hh[3];
            int e = hh[4], f = hh[5], g = hh[6], h = hh[7];
            for (int i = 0; i < 64; i++) {
                int t1 = h + (Integer.rotateRight(e, 6) ^ Integer.rotateRight(e, 11) ^ Integer.rotateRight(e, 25)) + ((e & f) ^ ((~e) & g)) + K[i] + w[i];
