...
public class FullSoftFloat {
    ...
    public static int add(int a, int b) {
        ...
        int sign = sign(a), mantissa = mantissa(a) << 3;
        int signB = sign(b);

        int mantissaB = mantissa(b) << 3;
        if (shift >= 32) {
            mantissaB = mantissaB != 0 ? 1 : 0;
        } else {
            mantissaB = shiftRightSticky(mantissaB, shift);
        }
        if (sign == signB) {
            mantissa += mantissaB;
        } else {
        ...
        if (exponent <= 0) {
            shift = 1 - exponent;
            mantissa = shiftRightSticky(mantissa, shift);
            exponent = 0;
        }
        if (exponent >= 0xff) {
            return sign | 0x7f800000;
        }
        int guard  = (mantissa >> 2) & 1;
        int round  = (mantissa >> 1) & 1;
        int sticky = mantissa & 1;
        mantissa >>>= 3;
        if (guard != 0 &&
                (round != 0 || sticky != 0 || (mantissa & 1) != 0)) {
            mantissa++;
        }
        if (mantissa >= (1 << 24)) {
            mantissa >>>= 1;
    ...
    private static int shiftRightSticky(int x, int shift) {
        if (shift <= 0) {
            return x;
        }
        if (shift >= 32) {
        ...
        int lost = x & ((1 << shift) - 1);
        return (x >>> shift) | (lost != 0 ? 1 : 0);
    }
