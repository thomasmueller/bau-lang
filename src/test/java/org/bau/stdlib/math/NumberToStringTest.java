...
public class NumberToStringTest {
    ...
    public void doubleTest() {
        ...
        for (double n = 1.0; n < Double.POSITIVE_INFINITY; n *= 10.0) {
            ...
            String got = SimpleMath.doubleToString(n);
    ...
    public void longTest() {
        ...
        for (int i = 0; i < 100_000; i++) {
            for (long max = 10; max > 0; max *= 10) {
                ...
                String got = SimpleMath.longToString(x);
                assertEquals(expected, got);
            }
        }
        String expected = Long.toString(Long.MIN_VALUE);
        String got = SimpleMath.longToString(Long.MIN_VALUE);
        assertEquals(expected, got);
        expected = Long.toString(Long.MAX_VALUE);
        got = SimpleMath.longToString(Long.MAX_VALUE);
        assertEquals(expected, got);
    }
    ...
    public void performanceLong() {
        ...
        for (int i = 0; i < 10_000; i++) {
            for (long max = 10; max > 0; max *= 10) {
                ...
                String s = SimpleMath.longToString(x);
                dummy += s.hashCode();
                count++;
            }
        }
    ...
    public void performanceDouble() {
        ...
        for (int i = 0; i < 10_000; i++) {
            for (long max = 10; max > 0; max *= 10) {
                ...
                String s = SimpleMath.doubleToString(x);
                dummy += s.hashCode();
                count++;
            }
        }
